...
if ( ! empty( $images ) ) {
	...
	foreach ( $images as $k => $img ) {
		...
		if ( get_option( 'woo_resize' ) == 'true' && $settings['use_timthumb'] == 'true' ) {
			...
			$timthumb_args = 'src=' . $src[0] . '&w=' . $settings['width'] . '&q=90&zc=1';
			if ( $settings['use_height'] == true ) { $timthumb_args .= '&h=' . $settings['height']; }
			$timthumb_args = apply_filters( 'woo_post_gallery_timthumb_args', $timthumb_args, $img->ID );
			
			$img_url = get_template_directory_uri() . '/thumb.php?' . $timthumb_args;
