...
package ca.newcommerce.youtube.webservice
{
	...
	{	
		...
		protected function runLoader(request:URLRequest, doComplete:Function, wrapper:Object):Number
		{
			...
			loader.addEventListener(NetStatusEvent.NET_STATUS, doConnectionStatus);
		...
		protected function doRawUrlLoaded(evt:Event):void
		{
			...
			var wrapper:Object = getWrapper(evt.target as URLLoader);
			dispatchEvent(new YouTubeEvent(YouTubeEvent.RAW_URL_DATA_RECEIVED, wrapper, null));
		}
		...
		/**
		* event handler for NetStatusEvent.NET_STATUS on URLLoader
		* @param	evt NetStatusEvent object
		*/
		protected function doConnectionStatus(evt:NetStatusEvent):void
		{
			//trace("WSClient.doConnectionStatus("+evt.toString());
		}
